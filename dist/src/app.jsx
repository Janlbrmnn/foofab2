...
const App = () => {
  ...
  <nav className="nav" data-screen-label={page}>
      <button className="nav-brand" onClick={() => setPage('landing')} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <Wordmark height={22} />
      </button>
      <div className="nav-links">
        <button onClick={() => setPage('hub')}>about</button>
        <button className={page === 'config' || page === 'inspire' || page === 'collab' ? 'active' : ''}
      onClick={() => setPage('hub')}>create</button>
        <button onClick={() => setPage('landing')}>impressum</button>
      </div>
    </nav>;
