...
const Inspire = ({ onConfig }) => {
  ...
  return (
    <div style={{ padding: '80px 32px 48px', maxWidth: 1440, margin: '0 auto' }}>
      {/* header */}
      <div style={{ paddingBottom: 20, borderBottom: '1px solid var(--line)', marginBottom: 32, display: 'flex', justifyContent: 'space-between', alignItems: 'end' }}>
        <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
        {filtered.map((t, i) => (
          <article key={t.k} className="fade-up" style={{
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'calc(10px * var(--scale))', color: 'var(--fg-3)', letterSpacing: '0.15em' }}>
              <span>[ {t.k} ]</span>
              <span>#{t.tag}</span>
            </div>
            ...
            <div>
              <div style={{ fontSize: 'calc(16px * var(--scale))', lineHeight: 1.25, letterSpacing: '-0.01em', marginBottom: 6 }}>
                {t.title}
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 10, borderTop: '1px dashed var(--line)', fontSize: 'calc(11px * var(--scale))' }}>
      ...
      <div style={{ marginTop: 56, paddingTop: 24, borderTop: '1px solid var(--line)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {BRIEFS.map((b, i) => (
            ...
            >
              <span style={{ color: 'var(--fg-3)', fontSize: 'calc(10px * var(--scale))', letterSpacing: '0.2em' }}>[ {b.kw} ]</span>
              <span style={{ fontSize: 'calc(15px * var(--scale))' }}>{b.title}</span>
              <span style={{ color: 'var(--fg-2)', fontSize: 'calc(11px * var(--scale))' }}>{b.bounty}</span>
              <Arrow dir="right" />
            </button>
          ))}
        </div>
